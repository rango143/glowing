...
for puncturing = {'1/2','2/3','5/6'}
    ...
    EbNo_Output = EbNo_Input_ray +10*log10(rate*log2(4));%code Eb/No rate
    ...
    for n = 1:length(EbNo_Output)
        ...
        while(BER_Vec(2,n) < target_Error)&&(BER_Vec(3,n)<max_Num_Trans)
            ...
            Rayleigh = sqrt(0.5*((randn(1,length(mod_data))).^2+(randn(1,length(mod_data))).^2));%Rayleigh channel
            data_Ray = mod_data.*Rayleigh';
            channel_output = step(Q_Channel,data_Ray);%through AWGN
    ...
    coded_Q_Ray = BER_Vec(1,:);
    release(Conv_Enc);
    release(Q_VitDec);
    
    banner = {'uncoded QPSK '};
    uncoded_Q = berawgn(EbNo_Input_ray,'psk',4,'diff');%Returns the value of an unencoded psk that was consistently detected on the AWGN channel.
   

 
    if rate == 1/2
       semilogy(EbNo_Input_ray,coded_Q_Ray,'g-o');%plot uncoded QPSK
       hold on;
    end
    if rate == 2/3
       Line2=semilogy(EbNo_Input_ray,coded_Q_Ray,'b-o');
       hold on;

    end
    if rate == 5/6
       Line3 = semilogy(EbNo_Input_ray,coded_Q_Ray,'k-o');
    ...
    title('BER vs Eb/No for QPSK in Rayleigh fading');
    
    ylabel('BER');  
    
    hold on;
    BER_ray = [BER;coded_Q_Ray];
    save('BER_ray_compare','EbNo_Input_ray','coded_Q_Ray','uncoded_Q')%save results

end
legend('Rate=1/2','Rate=2/3','Rate = 5/6');